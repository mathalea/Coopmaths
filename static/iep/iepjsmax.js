...
iep.traiteBalise = function(ch,infoBalise,txt,debutLigne,y) {
  ...
  if ((ind === -1) || (ind > 0)) { // Il y a du texte
    ...
    stylespan += "font-size:" + infoBalise.taille + ";";
...
iep.iepDoc = function (svg, chdoc, autoStart) {
  ...
  // On corrige les atributs en doublon
  ...
  // On crée les instruements dès le début car àa chauque fois qu'un instruent est translaté, tourné, zoomé,
...
iep.iepDoc.prototype.initialise = function() {
  ...
  for (i = 0; i < svg.children.length; i++) {
    ...
    if (el != this.barreIcones)
      svg.children[i].setAttribute("visibility","hidden");
  }
  for (i = 0; i < this.actions.length; i++) {
...
iep.regle.prototype.montreGraduations = function(bvisible) {
  ...
  this.graduations.setAttribute("visibility", this.visible ? "visible" : "hidden");
}
/**
 * Fonction monrant la régle ou la cachant suivant la valeur de bvisible
 * @param {type} bvisible
 */
...
iep.actionEcrireTexte.prototype.creegLatex = function() {
  ...
  try {
    ...
    if ((this.couleurFond != null) || (this.couleurCadre != null)) {
      ...
      g.setAttribute("visible", "visible");  
