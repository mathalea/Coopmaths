...
function Exercice_additionner_des_fractions(){
	Exercice.call(this); // Héritage de la classe Exercice()
	this.sup = false ; // Avec ou sans relatifs
	...
	this.nouvelle_version = function(){
		this.liste_questions = []; // Liste de questions
		this.liste_corrections = []; // Liste de questions corrigées
		let type_de_questions_disponibles = ['ppcm']//,'premiers_entre_eux','multiples']
		let liste_type_de_questions = combinaison_listes(type_de_questions_disponibles,this.nb_questions) // Tous les types de questions sont posées mais l'ordre diffère à chaque "cycle"
		for (let i = 0, a, b, c, d, k, k1, k2, texte, texte_corr, type_de_questions, cpt=0; i < this.nb_questions;i++) {
			type_de_questions = liste_type_de_questions[i];
			switch (type_de_questions){
				case 'ppcm' :
					let liste_couples_de_denominateurs = [[6,9],[4,6],[8,12],[9,12],[10,15],[10,25],[6,21]]
					...
					k2 = ppcm(b,d)/d;
					texte = `$${tex_fraction(a,b)}+${tex_fraction(c,d)}=$`;
					texte_corr = `$${tex_fraction(a,b)}+${tex_fraction(c,d)}=${tex_fraction(a+mise_en_evidence('\\times'+k1),b+mise_en_evidence('\\times'+k1))}+${tex_fraction(c+mise_en_evidence('\\times'+k2),d+mise_en_evidence('\\times'+k2))}`
					texte_corr += `=${tex_fraction(a*k1,b*k1)}+${tex_fraction(c*k2,d*k2)}=${tex_fraction(a*k1+`+`+c*k2,b*k1)}=${tex_fraction(a*k1+c*k2,b*k1)}$`;
					// Est-ce que le résultat est simplifiable ?
					let s = pgcd(a*k1+c*k2,b*k1); 
					if (s!=1) {
						if (a*k1+c*k2%b*k1==0) { //si le résultat est entier
							texte_corr += `$=${a*k1+c*k2/b*k1}$`
						} else {
							texte_corr +=`$=${tex_fraction(Algebrite.eval((a*k1+c*k2)/s)+mise_en_evidence('\\times'+s),Algebrite.eval(c*k1/s)+mise_en_evidence('\\times'+s))}=${tex_fraction(Algebrite.eval((a*k1+c*k2)/s),Algebrite.eval(c*k1/s))}$`
						}
					} 
