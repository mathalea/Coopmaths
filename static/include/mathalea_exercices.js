		...
		this.nouvelle_version = function(numero_de_l_exercice){
			...
			if (this.sup==1){  // calcul direct de AM et BC : pas de calcul intermédiaire de AN
			 ...
			 {  // Calcul de AN nécessaire avant de calculer AM et BC
				...
				texte = `$${s1}$, $${s2}$ et $${s3}$ sont trois point distincts. $${s4}$&isin;$[${s1+s2}]$ et $${s5}$&isin;$[${s1+s3}]$ tel que les droites $(${s4+s5})$ et $(${s2+s3})$ sont parallèles.</br> $${s1+s2}=${s12}$ cm, $${s1+s3}=${s13}$ cm, $${s4+s5}=${s45}$ cm et $${s1+s5}=${s15}$ cm.`
